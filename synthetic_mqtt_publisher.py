...
This is the **combined, single-process** publisher — one script, one
``SyntheticBatteryGenerator``, publishing both the twin and ECU streams
from the same run loop. For two genuinely independent publishers (closer
to how the real Simulink/twin and NXP MPC5744P/ECU sources are actually
wired up — optionally on two different machines), use
``synthetic_mqtt_publisher_twin.py`` + ``synthetic_mqtt_publisher_ecu.py``
instead; both are documented to stay correlated with each other under a
shared ``--seed``.
